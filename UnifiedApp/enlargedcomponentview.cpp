...
void SubsystemHealthBar::paintEvent(QPaintEvent* /*event*/)
{
    ...
    // Background card
    QColor cardBg(30, 33, 40);
    p.setPen(QPen(QColor(50, 54, 65), 1));
    p.setBrush(cardBg);
...
void HealthTrendChart::paintEvent(QPaintEvent* /*event*/)
{
    ...
    // Background
    p.setPen(Qt::NoPen);
    p.setBrush(QColor(22, 24, 32));
    p.drawRoundedRect(0, 0, w, h, 8, 8);

    // Grid lines
    p.setPen(QPen(QColor(45, 49, 60), 1, Qt::DotLine));
    for (int i = 0; i <= 4; i++) {
        int y = chartTop + chartH * i / 4;
        p.drawLine(chartLeft, y, chartRight, y);
        // Y-axis labels
        p.setPen(QColor(120, 124, 135));
    ...
    // Draw area fill under the trend line
    ...
    if (n >= 2) {
        ...
        QColor areaColor = m_dataPoints.last().color;
        areaColor.setAlpha(30);
        p.setPen(Qt::NoPen);
    ...
    if (n > 0) {
        ...
        p.drawText(QRectF(chartRight - 60, chartTop, 60, 22),
...
EnlargedComponentView::EnlargedComponentView(const QString& componentId,
                                             ...
                                             const QStringList& subsystemNames,
                                             QWidget* parent)
    : QWidget(parent)
    ...
    , m_subsystemNames(subsystemNames)
    , m_updateCount(0)
    , m_statusChanges(0)
    , m_healthSum(0)
{
...
void EnlargedComponentView::setupUI()
{
    ...
    // Title - get display name from registry
    ...
    idLabel->setStyleSheet(
        "color: #00BCD4; font-size: 10px; font-weight: 500;"
    ...
    m_healthValueLabel->setStyleSheet(
        ...
        "padding: 4px 12px; background: rgba(21,101,192,0.1);"
        "border-radius: 6px;");
    ...
    m_displayComponent->setFlag(QGraphicsItem::ItemIsMovable, false);
    m_displayComponent->setPos(30, 20);
    ...
    m_componentView->setBackgroundBrush(QBrush(QColor(18, 20, 26)));
    m_componentView->setMinimumHeight(220);
    m_componentView->setMaximumHeight(350);
    m_componentView->setFrameShape(QFrame::NoFrame);
    m_componentView->setStyleSheet(
        "border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 8px;");
    m_componentView->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    m_componentView->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    ...
    for (const QString& name : m_subsystemNames) {
        SubsystemHealthBar* bar = new SubsystemHealthBar(name, 100, QColor("#4CAF50"), subsysContainer);
        subsysLayout->addWidget(bar);
        m_subsystemBars[name] = bar;
    }
    ...
    QLabel* rightTitle = new QLabel("COMPONENT ANALYTICS", rightPanel);
    ...
    auto createStatCard = [&](const QString& label, const QString& value, int row, int col) -> QLabel* {
        ...
        card->setStyleSheet(
            "background: rgba(18, 20, 26, 0.9); border: 1px solid rgba(255, 255, 255, 0.06);"
            "border-radius: 8px;");
        ...
        lbl->setStyleSheet(
            "color: #5f6368; font-size: 9px; font-weight: 600;"
    ...
    for (const QString& name : m_subsystemNames) {
        ...
        nameLabel->setStyleSheet(
            "color: #c4c7cc; font-size: 10px; background: transparent; border: none;");
...
void EnlargedComponentView::updateSubsystemHealth(const QString& subName, int health, const QColor& color)
{
