...
Item {
    ...
    // ── Layout: charts canvas on top, command log below ──────────
    Column {
        anchors.fill: parent
        spacing: 0
        ...
        Canvas {
            ...
            onPaint: {
                ...
                var donutR  = Math.max(4, Math.min(cw * 0.36, 62))
                var donutCx = pad + donutR + 4
                var donutCy = y + donutR + 4
                var donutIn = donutR * 0.58
                ...
                ctx.beginPath(); ctx.arc(donutCx,donutCy,donutIn,0,Math.PI*2)
                ctx.fillStyle=ap.bgCol; ctx.fill()
                } // end donut guard

                var gPct = total>0 ? Math.round(h*100/total) : 0
                ctx.fillStyle=ap.healthyCol; ctx.font="bold 16px 'Segoe UI'"
                ctx.textAlign="center"
                ctx.fillText(gPct+"%", donutCx, donutCy+5)
                ctx.fillStyle=ap.dimCol; ctx.font="8px 'Segoe UI'"
                ctx.fillText("HEALTH", donutCx, donutCy+16)

                // Legend beside donut
                var lx2=donutCx+donutR+12, ly2=donutCy-donutR+6
                var lbls2=[["Healthy",h,ap.healthyCol],["Warning",w2,ap.warningCol],
                           ["Critical",c,ap.critCol], ["No Data",u,ap.unknownCol]]
                for (var li=0; li<4; li++) {
                    ctx.fillStyle=lbls2[li][2]; ctx.fillRect(lx2,ly2+li*18,9,9)
                    ctx.fillStyle=ap.textCol; ctx.font="10px 'Segoe UI'"; ctx.textAlign="left"
                    ctx.fillText(lbls2[li][0]+"  "+lbls2[li][1], lx2+13, ly2+li*18+9)
                }
                ...
                // ── Per-quadrant bars ─────────────────────────────
                ctx.fillStyle=ap.accentCol; ctx.font="bold 10px 'Segoe UI'"
                ...
                for (var qi=0; qi<4 && qi<ap.model.quadrantCount; qi++) {
                    ...
                    var qcol= qv<0?ap.unknownCol:qs[2]>0?ap.critCol:qs[1]>0?ap.warningCol:qs[0]>0?ap.healthyCol:ap.unknownCol
                    ctx.fillStyle=ap.dimCol; ctx.font="9px 'Segoe UI'"
                    ctx.textAlign="left"; ctx.fillText(qLbls[qi], pad, y+11)
                    ctx.fillStyle=dark?"rgba(255,255,255,0.06)":"rgba(0,0,0,0.06)"
                    ctx.fillRect(pad+36, y, barMaxW, 12)
                    ctx.fillStyle=qcol; ctx.fillRect(pad+36, y, barMaxW*(qpct/100), 12)
                ...
                if (hist && hist.length>1) {
                    ...
                    gr2.addColorStop(0,dark?"rgba(0,232,122,0.30)":"rgba(0,168,84,0.20)")
                ...
                ctx.fillStyle=ap.dimCol;ctx.font="8px 'Segoe UI'"
                ctx.textAlign="right";ctx.fillText("100%",spX-2,y+8)
                ...
                ctx.beginPath();ctx.moveTo(pad,y);ctx.lineTo(width-pad,y);ctx.stroke()
                y+=8
                var pktStr=ap.udpRecv?("PKT: "+ap.udpRecv.packetsReceived):""
                var updStr=ap.model?ap.model.lastUpdateTime:""
                ctx.fillStyle=ap.dimCol;ctx.font="9px 'Segoe UI'"
                ctx.textAlign="left";ctx.fillText(pktStr,pad,y+10)
                ctx.textAlign="right";ctx.fillText(updStr,width-pad,y+10)
                ctx.textAlign="left"
            }
        }
        ...
        Rectangle {
            width: parent.width
            height: ap.height - chartsCanvas.height - 1 - 30
            color: ap.bgCol
            clip: true
                ...
                delegate: Rectangle {
                    ...
                    Rectangle {
                        ...
                        color: {
                            ...
                            if (s === 1) return ap.healthyCol   // ACK OK
                            if (s === 2) return ap.warningCol   // BUSY
                            if (s === 3) return ap.critCol      // ERROR
                            return ap.accentCol                  // PENDING
                        }
                    }

                    Column {
                        anchors { left: parent.left; right: parent.right; leftMargin: 10; topMargin: 6 }
                        spacing: 2

                        // Row 1: command name + element ID
                        Row {
                            spacing: 6
                            Text {
